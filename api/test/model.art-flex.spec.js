...
describe('model.art-flex', () => {
  ...
  //   let rec = await ArtFlex.create({title: 'number 201', owner: addr1._id, artId: 201});
  //   rec = await rec.save();
  //   rec = await ArtFlex.find({artId: 201});
  //   rec = await rec[0].populate('_fields.address')
  //   let obj = rec.objectGet();
  //   assert.equal(obj.title, 'number 201');
  //   assert.equal()
  // });
